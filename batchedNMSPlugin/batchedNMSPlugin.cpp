...
const char* NMS_PLUGIN_NAME{"DY_BatchedNMS"};
} // namespace
    ...
    int outputIndex, const DimsExprs* inputs, int nbInputs, IExprBuilder& exprBuilder)
{
    ...
    ASSERT(inputs[1].nbDims == 3
        || (inputs[1].nbDims == 4
               && exprBuilder.operation(DimensionOperation::kEQUAL, *inputs[1].d[3], *exprBuilder.constant(1))));
    // boxesSize: number of box coordinates for one sample
    boxesSize = inputs[0].d[1]->getConstantValue() * inputs[0].d[2]->getConstantValue() * inputs[0].d[3]->getConstantValue();

    // scoresSize: number of scores for one sample
    scoresSize = inputs[1].d[1]->getConstantValue() * inputs[1].d[2]->getConstantValue();
    // num_detections
    if (outputIndex == 0)
    {
        DimsExprs dim0{};
        dim0.nbDims = 1;
        dim0.d[0] = inputs[0].d[0];
        return dim0;
    }
    // nmsed_boxes
    if (outputIndex == 1)
    {
        ...
        dim0.nbDims = 3;
        dim0.d[0] = inputs[0].d[0];
        dim0.d[1] = exprBuilder.constant(param.keepTopK);
        dim0.d[2] = exprBuilder.constant(4);
        return dim0;
    }
    // nmsed_scores or nmsed_classes
    DimsExprs dim1{};
    dim1.nbDims = 2;
    dim1.d[0] = inputs[0].d[0];
    dim1.d[1] = exprBuilder.constant(param.keepTopK);
    return dim1;
}
